...
Deno.serve(async (request) => {
  ...
  const { data: checkout } = await admin.from("billing_checkout_sessions").select("id,agency_id,status,amount,transaction_nsu").eq("provider", "infinitepay").eq("order_nsu", orderNsu).maybeSingle();
  ...
  const response = await fetch("https://api.checkout.infinitepay.io/payment_check", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ handle, order_nsu: orderNsu, transaction_nsu: transactionNsu, slug }) });
  ...
  const paymentUpdate = await admin.from("billing_checkout_sessions").update({ status: "paid", completed_at: new Date().toISOString(), transaction_nsu: transactionNsu, invoice_slug: slug, receipt_url: receiptUrl || null, capture_method: body.capture_method || captureMethod || null, paid_amount: Number(body.paid_amount || body.amount || 0) / 100, provider_payload: { payment_check: body }, updated_at: new Date().toISOString() }).eq("id", checkout.id);
  if (paymentUpdate.error) return json({ error: paymentUpdate.error.message }, 500);
  ...
  await admin.from("billing_events").upsert({ provider: "infinitepay", provider_event_id: transactionNsu, agency_id: checkout.agency_id, event_type: "payment_confirmed", payload: body, processing_status: "processed", processed_at: new Date().toISOString(), last_error: null }, { onConflict: "provider,provider_event_id" });

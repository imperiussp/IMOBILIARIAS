  ...
  for (const entry of body?.entry || []) {
    for (const change of entry?.changes || []) {
      ...
      for (const status of value?.statuses || []) {
        ...
        if (!providerMessageId || !["sent", "delivered", "read", "failed"].includes(statusName)) {
          ignored++; continue;
        }

        const attempt = await admin.from("buyer_outreach_delivery_attempts")
        ...
        if (!attempt.data) { ignored++; continue; }
        ...
        if (statusName === "failed") {
          if (!["delivered", "read"].includes(current)) {
            patch.status = "failed";
            patch.error_message = clean(status?.errors?.[0]?.title || status?.errors?.[0]?.message || "Falha informada pela Meta.", 1200);
            shouldUpdate = true;
          }
        ...
        if (shouldUpdate) {
          const update = await admin.from("buyer_outreach_delivery_attempts").update(patch).eq("id", (attempt.data as any).id);
        ...
        if (statusName === "failed" && !["delivered", "read"].includes(current)) {
          const opportunityUpdate = await admin.from("buyer_property_opportunities")
            .update({ status: "failed", last_error: patch.error_message, updated_at: now })
            .eq("id", (attempt.data as any).opportunity_id);
          if (opportunityUpdate.error) return json({ error: opportunityUpdate.error.message }, 500);
        }
        processedStatuses++;
      }
      ...
      for (const message of value?.messages || []) {
        ...
        if (responseKind === "opt_out") {
          const permissionUpdate = await admin.from("lead_contact_permissions")
            .update({
            ...
            .eq("agency_id", attempt.agency_id)
            .eq("lead_id", attempt.lead_id);
          if (permissionUpdate.error) return json({ error: permissionUpdate.error.message }, 500);
        }

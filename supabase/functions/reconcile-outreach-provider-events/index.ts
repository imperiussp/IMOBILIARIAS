...
function providerTimestamp(value:unknown){const seconds=Number(value);if(!Number.isFinite(seconds)||seconds<=0)return new Date().toISOString();const date=new Date(seconds*1000);return Number.isNaN(date.getTime())?new Date().toISOString():date.toISOString();}
...
Deno.serve(async(request)=>{
  ...
  const suppliedSecret=request.headers.get("x-platform-maintenance-secret")||"";
  if(!suppliedSecret)return json({error:"unauthorized"},401);
  const admin=createClient(supabaseUrl,serviceKey,{auth:{persistSession:false}});
  const verified=await admin.rpc("verify_platform_maintenance_secret",{p_secret:suppliedSecret});
  if(verified.error||verified.data!==true)return json({error:"unauthorized"},401);

  const inbox=await admin.from("outreach_provider_event_inbox").select("id,provider,provider_event_id,provider_message_id,event_type,payload,processing_attempts").is("processed_at",null).is("abandoned_at",null).order("received_at",{ascending:true}).limit(100);
  ...
  for(const event of inbox.data||[]){
    try{
      ...
      if(!attemptResult.data){await admin.from("outreach_provider_event_inbox").update({processing_attempts:Number(event.processing_attempts||0)+1,last_error:"Tentativa ainda não correlacionada."}).eq("id",event.id);deferred++;continue;}
      const attempt:any=attemptResult.data;const now=new Date().toISOString();
      if(event.event_type==="meta_message"){
        const message:any=event.payload;const responseText=extractMetaText(message);const responseKind=classifyResponse(responseText);const receivedAt=providerTimestamp(message?.timestamp);
        ...
        if(responseKind==="opt_out"){const permission=await admin.from("lead_contact_permissions").update({automated_property_alerts_allowed:false,whatsapp_allowed:false,revoked_at:receivedAt,updated_at:receivedAt}).eq("agency_id",attempt.agency_id).eq("lead_id",attempt.lead_id);if(permission.error)throw new Error(permission.error.message);}
      }else{
        const isMeta=event.provider==="meta_whatsapp";const raw:any=event.payload;const mapped=isMeta?clean(raw?.status,40).toLowerCase():event.event_type==="email.sent"?"sent":event.event_type==="email.delivered"?"delivered":event.event_type==="email.opened"?"read":["email.failed","email.bounced","email.complained","email.suppressed"].includes(event.event_type)?"failed":"";
        if(!["sent","delivered","read","failed"].includes(mapped))throw new Error("Evento não suportado para reconciliação.");
        const occurredAt=isMeta?providerTimestamp(raw?.timestamp):clean(raw?.created_at,80)||now;const current=clean(attempt.status,40);const rank:Record<string,number>={prepared:0,sending:1,sent:2,delivered:3,read:4};const patch:Record<string,unknown>={provider_payload:raw};
        if(mapped==="failed"&&!["delivered","read"].includes(current)){patch.status="failed";patch.error_message=isMeta?clean(raw?.errors?.[0]?.title||raw?.errors?.[0]?.message||"Falha informada pela Meta.",1200):clean(raw?.data?.bounce?.message||raw?.data?.reason||raw?.data?.error||`Falha ${event.event_type}`,1200);}else if(mapped!=="failed"&&(rank[mapped]??-1)>(rank[current]??-1)){patch.status=mapped;if(mapped==="sent")patch.sent_at=occurredAt;if(mapped==="delivered")patch.delivered_at=occurredAt;if(mapped==="read"){patch.read_at=occurredAt;patch.delivered_at=occurredAt;}}
        ...
        if(mapped==="failed"&&!["delivered","read"].includes(current)){const opp=await admin.from("buyer_property_opportunities").update({status:"failed",last_error:patch.error_message||"Falha do provedor.",updated_at:occurredAt}).eq("id",attempt.opportunity_id);if(opp.error)throw new Error(opp.error.message);}
        if(!isMeta&&["email.bounced","email.complained","email.suppressed"].includes(event.event_type)){const permission=await admin.from("lead_contact_permissions").update({email_allowed:false,automated_property_alerts_allowed:false,revoked_at:occurredAt,updated_at:occurredAt}).eq("agency_id",attempt.agency_id).eq("lead_id",attempt.lead_id);if(permission.error)throw new Error(permission.error.message);}
      }
      await admin.from("outreach_provider_event_inbox").update({processed_at:now,processing_attempts:Number(event.processing_attempts||0)+1,last_error:null}).eq("id",event.id);processed++;

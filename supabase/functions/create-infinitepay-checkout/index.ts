...
Deno.serve(async (request) => {
  ...
  let baseAmount = chargeType === "implementation"
    ? numberValue(plan.implementation_fee)
    : numberValue(billingCycle === "annual" ? plan.annual_price : plan.monthly_price);
  ...
  // O desconto manual é consumido apenas pela próxima cobrança do plano.
  // A taxa de implantação continua independente; no anual ela é cortesia integral.
  if (chargeType === "subscription") {
    const { data: discount } = await admin.from("agency_billing_discounts")
      .select("id,base_amount,final_amount,discount_percent,valid_until,created_at")
  ...
  const reusableQuery = admin.from("billing_checkout_sessions")
    .select("id,order_nsu,checkout_url,created_at,amount")
    .eq("agency_id", agencyId)
    .eq("plan_id", planId)
  ...
  if (reusable?.checkout_url) {
    return json({ checkout_url: reusable.checkout_url, checkout_id: reusable.id, order_nsu: reusable.order_nsu, reused: true, charge_type: chargeType, base_amount: baseAmount, amount, discount_percent: discountPercent, implementation_free: implementationWaived });
  }
  ...
  const description = chargeType === "implementation"
    ? `LENOY IMOBILIÁRIAS — Implantação · ${plan.name}`
    : `LENOY IMOBILIÁRIAS — ${plan.name} (${billingCycle === "annual" ? "anual" : "mensal"})`;
  ...
  const created = await admin.from("billing_checkout_sessions").insert({
    ...
    provider_payload: { request: checkoutPayload, pricing: { base_amount: baseAmount, final_amount: amount, discount_percent: discountPercent, charge_type: chargeType, implementation_waived: implementationWaived } },
  });

...
Deno.serve(async (request) => {
  ...
  const membership = await userClient.from("agency_memberships")
    .select("role")
  ...
  const redirectUrl = `${siteUrl}/admin/?pagamento=retorno&order_nsu=${encodeURIComponent(orderNsu)}`;
  ...
  const checkoutPayload = {
    ...
    customer: {
      name: agency.name,
      email: userData.user.email || agency.email || undefined,
    },
  };
  ...
  const created = await admin.from("billing_checkout_sessions").insert({
    ...
    provider_payload: { request: checkoutPayload },
  });
  ...
  try {
    ...
    await admin.from("billing_checkout_sessions").update({
      ...
      provider_payload: { request: checkoutPayload, response: body },
    ...
    await admin.from("billing_checkout_sessions").update({
      status: "failed",
      provider_payload: { request: checkoutPayload, error: error instanceof Error ? error.message : String(error) },
      updated_at: new Date().toISOString(),
    }).eq("id", sessionId);

  ...
  for (const raw of queue.data || []) {
    ...
    try {
      ...
      if (!aiMessage) {
        ...
        await admin.from("buyer_property_opportunities").update({ ai_message: aiMessage, ai_provider: aiModel, channel, updated_at: new Date().toISOString() }).eq("id", item.id);
      }
      ...
      if (attempt.error) {
        if ((attempt.error.message || "").toLowerCase().includes("horário silencioso")) { deferred++; continue; }
        if ((attempt.error.message || "").toLowerCase().includes("limite mensal")) {
      ...
      await admin.from("buyer_outreach_delivery_attempts").update({ status: "sent", sent_at: now, provider_message_id: providerMessageId, provider_payload: deliveryBody, error_message: null }).eq("id", attemptId);
      await admin.from("buyer_property_opportunities").update({ status: "sent", channel, ai_message: aiMessage, ai_provider: aiModel || "prepared", sent_at: now, provider_message_id: providerMessageId, last_error: null, updated_at: now }).eq("id", item.id);
      sent++;
    } catch (error) {

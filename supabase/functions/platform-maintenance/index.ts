...
Deno.serve(async(request)=>{
  ...
  results.push(await callFunction("reconcile-outreach-provider-events",{"x-platform-maintenance-secret":suppliedSecret}));
  ...
  if(pendingDomains.error){results.push({name:"verify-custom-domains",ok:false,status:500,error:pendingDomains.error.message});}
  else if((pendingDomains.count||0)===0){results.push({name:"verify-custom-domains",ok:true,skipped:true,reason:"no_pending_custom_domains"});}
  else if(!domainSecret){results.push({name:"verify-custom-domains",ok:false,configuration_error:true,error:"DOMAIN_VERIFY_SECRET não configurado",pending_domains:pendingDomains.count||0});}
  else {results.push(await callFunction("verify-custom-domains",{"x-domain-verify-secret":domainSecret}));}
  ...
  } else {results.push({name:"push-broker-notifications",ok:true,skipped:true,reason:"release_gate_disabled"});}
  ...
  } else {results.push({name:"process-buyer-opportunities",ok:true,skipped:true,reason:"release_gates_disabled"});}
  ...
  const payload={ok:failed===0,environment_mode:controls.environment_mode||"unknown",maintenance_mode:controls.maintenance_mode===true,processed_at:new Date().toISOString(),failed,configuration_issues:configurationIssues,results};

  try {await admin.from("platform_maintenance_runs").insert({started_at:startedAt,finished_at:payload.processed_at,success:payload.ok,failed_tasks:failed,result:payload});} catch {}

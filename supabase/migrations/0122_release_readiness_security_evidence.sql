-- Pré-voo v3: inclui isolamento multi-tenant e evidências de homologação.
...
with controls as (
  ...
  limit 1
), successful_maintenance as (
  select max(started_at) as last_successful_at
  from public.platform_maintenance_runs
  where success=true
), provider_queue as (
  select
  ...
  select
    count(*) filter(where required_for_production=true)::int as required_total,
    count(*) filter(where required_for_production=true and validated=true)::int as required_done,
    count(*) filter(where required_for_production=true and validated=false)::int as required_pending,
    count(*) filter(where validated=true)::int as total_done,
    count(*)::int as total
...
select * from (
  ...
  select 20,'homologation_guard','Freios externos coerentes com o ambiente',
    ...
    case when c.environment_mode='production' then 'Produção pode liberar integrações individualmente conforme necessidade.'
  ...
  union all
  select 55,'maintenance_success','Manutenção bem-sucedida nas últimas 24 horas',
    (sm.last_successful_at is not null and sm.last_successful_at >= now()-interval '24 hours'),
    case when sm.last_successful_at is null then 'Nenhuma manutenção bem-sucedida registrada.'
         when sm.last_successful_at < now()-interval '24 hours' then 'Última manutenção bem-sucedida está desatualizada.'
         else 'Último sucesso: '||to_char(sm.last_successful_at at time zone 'America/Sao_Paulo','DD/MM/YYYY HH24:MI') end,
    case when c.environment_mode='production' then 'required' else 'recommended' end from controls c cross join successful_maintenance sm
  union all
  ...
  union all
  select 65,'tenant_security','Isolamento estrutural multi-imobiliária aprovado',coalesce(ts.critical,0)=0,
    (coalesce(ts.ok,0)||' tabela(s) OK; '||coalesce(ts.critical,0)||' crítica(s); '||coalesce(ts.missing,0)||' ausente(s).')::text,
    'required' from tenant_security ts
  union all
  select 68,'release_validations','Testes obrigatórios de homologação validados',coalesce(v.required_pending,0)=0,
    (coalesce(v.required_done,0)||'/'||coalesce(v.required_total,0)||' obrigatório(s) validado(s); '||coalesce(v.required_pending,0)||' pendente(s).')::text,
...
comment on view public.platform_homologation_readiness is 'Pré-voo v3 com manutenção bem-sucedida recente, segurança multi-tenant e evidências obrigatórias de homologação.';
comment on view public.platform_release_readiness_summary is 'Resumo numérico do pré-voo v3 da plataforma.';

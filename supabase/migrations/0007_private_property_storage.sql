...
create policy "public read published property storage" on storage.objects for select to anon, authenticated using (
  bucket_id = 'property-photos' and exists (
...
create policy "authorized upload own property storage" on storage.objects for insert to authenticated with check (
  bucket_id = 'property-photos' and exists (
    select 1 from public.properties p left join public.brokers b on b.id = p.broker_id
    where p.id = public.storage_property_id(name)
      and (public.is_admin() or (b.user_id = auth.uid() and b.active = true))
  )
);

create policy "authorized update own property storage" on storage.objects for update to authenticated using (
  bucket_id = 'property-photos' and exists (
    select 1 from public.properties p left join public.brokers b on b.id = p.broker_id
    where p.id = public.storage_property_id(name)
      and (public.is_admin() or (b.user_id = auth.uid() and b.active = true))
...
create policy "authorized delete own property storage" on storage.objects for delete to authenticated using (
  bucket_id = 'property-photos' and exists (
    select 1 from public.properties p left join public.brokers b on b.id = p.broker_id
    where p.id = public.storage_property_id(name)
      and (public.is_admin() or (b.user_id = auth.uid() and b.active = true))

alter table public.brokers add column if not exists address text;

create or replace view public.property_catalog as
select
  p.id,
  p.agency_id,
  coalesce(nullif(p.display_code, ''), p.code) as code,
  p.slug,
  p.title,
  p.description,
  p.purpose,
...
where p.publication_state = 'published'::public.publication_state
  and p.status = any (array['available'::public.property_status,'reserved'::public.property_status,'rented'::public.property_status,'sold'::public.property_status]);

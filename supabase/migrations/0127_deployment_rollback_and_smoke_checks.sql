-- Endurece a implantação com rollback e smoke test pós-deploy.
...
values
  ('health_endpoint_verified','Endpoint /api/health responde saudável no domínio publicado','web',true,true),
  ('post_deploy_smoke_passed','Smoke test pós-deploy concluído sem falhas críticas','operations',true,true),
  ('rollback_plan_verified','Plano de rollback documentado e executável','operations',false,true)
...
comment on function public.guard_platform_production_promotion() is 'Trava final de produção incluindo saúde web, smoke test, rollback, infraestrutura, evidências e isolamento multi-tenant.';

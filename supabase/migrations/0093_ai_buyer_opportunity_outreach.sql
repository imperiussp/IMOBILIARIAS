...
using (public.is_agency_manager(agency_id) or public.is_platform_admin())
with check (public.is_agency_manager(agency_id) or public.is_platform_admin());
...
create trigger lead_contact_permissions_validate before insert or update on public.lead_contact_permissions
for each row execute function public.validate_lead_contact_permission_tenant();
...
create trigger properties_queue_buyer_matches
...
from public.buyer_property_opportunities
group by agency_id;

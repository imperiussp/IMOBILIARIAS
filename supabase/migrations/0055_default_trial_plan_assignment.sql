...
begin
  ...
  if not exists (
    ...
    where s.agency_id = new.id
      and s.status in ('trial','active','past_due')
  ) then
    insert into public.agency_subscriptions (
      agency_id, plan_id, status, starts_at, renews_at, ends_at
    ) values (
      new.id, trial_plan_id, 'trial', now(), now() + make_interval(days => trial_days), now() + make_interval(days => trial_days)
    );
  end if;
...
-- Helper explícito para backfill/administrador; usa exatamente a mesma regra do trigger.
...
declare
  ...
  trial_days integer;
begin
  if exists (select 1 from public.agency_subscriptions s where s.agency_id = p_agency_id and s.status in ('trial','active','past_due')) then
    return;
  end if;
  ...
  if trial_plan_id is not null then
    insert into public.agency_subscriptions (agency_id, plan_id, status, starts_at, renews_at, ends_at)
    values (p_agency_id, trial_plan_id, 'trial', now(), now() + make_interval(days => trial_days), now() + make_interval(days => trial_days));
  end if;
end;
$$;

revoke all on function public.attach_default_trial_plan_to_agency_from_id(uuid) from public;
grant execute on function public.attach_default_trial_plan_to_agency_from_id(uuid) to authenticated;

-- Produção exige evidências explícitas dos testes críticos de homologação.
...
declare
  ...
  pending_required_validations integer:=0;
begin
  if new.environment_mode='production' and old.environment_mode is distinct from 'production' then
    ...
    select count(*)::integer into pending_required_validations
    from public.platform_release_validations
    where required_for_production=true and validated=false;

    if pending_required_validations > 0 then
      raise exception 'Produção bloqueada: existem % teste(s) obrigatório(s) de homologação sem evidência de validação.', pending_required_validations;
    end if;
  end if;
...
'Produção exige manutenção recente bem-sucedida, fila saudável, isolamento tenant estrutural aprovado e todos os testes críticos de homologação marcados como validados.';

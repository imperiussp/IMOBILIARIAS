...
function BrokerApp() {
  ...
  const chrome = <AppTopbar onMenu={() => setMenuOpen(true)} />;
  const menu = <BrokerMenu visible={menuOpen} active={screen} pending={pending} failed={failed} onClose={() => setMenuOpen(false)} onNavigate={navigate} onNew={openNew} onSignOut={() => void signOut()} />;

  if (screen === "published") return <SafeAreaView style={styles.screen}>{chrome}{menu}<PublishedProperties onClose={() => setScreen("home")} /></SafeAreaView>;
  if (screen === "leads") return <SafeAreaView style={styles.screen}>{chrome}{menu}<BrokerLeads onClose={() => setScreen("home")} /></SafeAreaView>;

  if (screen === "queue") return (
    <SafeAreaView style={styles.screen}>{chrome}{menu}<ScrollView contentContainerStyle={styles.content}>
  ...
  if (screen === "editor") return (
    <SafeAreaView style={styles.screen}>{chrome}{menu}<ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
      ...
      <View style={styles.formCard}>
        ...
        <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.stateStrip}>{states.map((uf) => <Pressable key={uf} style={[styles.stateChip, cityState === uf && styles.stateChipActive]} onPress={() => { setCityState(uf); setForm((current) => ({ ...current, city: "" })); setCityPickerOpen(true); setCitySearch(""); }}><Text style={[styles.stateChipText, cityState === uf && styles.stateChipTextActive]}>{uf}</Text></Pressable>)}</ScrollView>
        <Text style={styles.label}>Cidade</Text>
        <Pressable style={styles.citySelect} onPress={() => setCityPickerOpen((value) => !value)}><Text style={form.city ? styles.citySelectText : styles.cityPlaceholder}>{form.city ? parseCityValue(form.city).name : cityLoading ? "Carregando cidades..." : "Selecionar cidade"}</Text><Text style={styles.cityChevron}>{cityPickerOpen ? "⌃" : "⌄"}</Text></Pressable>
        {cityPickerOpen ? <View style={styles.cityPicker}><TextInput style={styles.citySearch} value={citySearch} onChangeText={setCitySearch} autoCapitalize="words" placeholder={`Buscar cidade em ${cityState}`} placeholderTextColor="#98a2ab" />
          <ScrollView style={styles.cityList} nestedScrollEnabled keyboardShouldPersistTaps="handled">
            {filteredCities.map((name) => <Pressable key={`${cityState}-${name}`} style={styles.cityOption} onPress={() => selectCity(name)}><Text style={styles.cityOptionName}>{name}</Text><Text style={styles.cityOptionUf}>{cityState}</Text></Pressable>)}
...
function AppTopbar({ onMenu }: { onMenu: () => void }) {
  return <View style={styles.appTopbar}><View><Text style={styles.appTopKicker}>LENOY</Text><Text style={styles.appTopTitle}>App do corretor</Text></View><Pressable style={styles.hamburger} onPress={onMenu} accessibilityLabel="Abrir menu"><Text style={styles.hamburgerText}>☰</Text></Pressable></View>;
}

function BrokerMenu(props: { visible: boolean; active: Screen; pending: number; failed: number; onClose: () => void; onNavigate: (screen: Screen) => void; onNew: () => void; onSignOut: () => void }) {
  const item = (screen: Screen, label: string, detail: string) => <Pressable key={screen} style={[styles.menuItem, props.active === screen && styles.menuItemActive]} onPress={() => props.onNavigate(screen)}><View style={{flex:1}}><Text style={[styles.menuItemTitle, props.active === screen && styles.menuItemTitleActive]}>{label}</Text><Text style={styles.menuItemDetail}>{detail}</Text></View>{props.active === screen ? <Text style={styles.menuActiveMark}>●</Text> : <Text style={styles.menuArrow}>›</Text>}</Pressable>;
  return <Modal visible={props.visible} transparent animationType="fade" onRequestClose={props.onClose}><View style={styles.menuModal}><Pressable style={styles.menuBackdrop} onPress={props.onClose} /><View style={styles.menuDrawer}><View style={styles.menuHead}><View><Text style={styles.menuBrand}>LENOY IMOBILIÁRIAS</Text><Text style={styles.menuTitle}>Menu do corretor</Text></View><Pressable style={styles.menuClose} onPress={props.onClose}><Text style={styles.menuCloseText}>×</Text></Pressable></View><ScrollView contentContainerStyle={styles.menuList}>{item("home","Início","Painel e atalhos")}<Pressable style={styles.menuItem} onPress={() => { props.onClose(); props.onNew(); }}><View style={{flex:1}}><Text style={styles.menuItemTitle}>Novo imóvel</Text><Text style={styles.menuItemDetail}>Cadastrar e fotografar</Text></View><Text style={styles.menuArrow}>＋</Text></Pressable>{item("published","Meus imóveis","Publicados e andamento")}{item("leads","Contatos","Leads e interessados")}{item("drafts","Rascunhos locais","Cadastros salvos no aparelho")}{item("queue","Sincronização",props.failed ? `${props.failed} erro(s) · ${props.pending} na fila` : `${props.pending} na fila`)}<Pressable style={styles.menuLogout} onPress={props.onSignOut}><Text style={styles.menuLogoutText}>Sair da conta</Text></Pressable></ScrollView></View></View></Modal>;
}

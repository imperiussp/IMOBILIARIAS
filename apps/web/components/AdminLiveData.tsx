...
type LiveProperty = {
  ...
  status: "available" | "reserved" | "rented" | "sold" | "inactive"; price: number; bedrooms?: number | null; bathrooms?: number | null; parking_spaces?: number | null;
  featured?: boolean; description?: string | null; address?: string | null; address_public?: boolean; created_at?: string; cities?: { name?: string; state_code?: string } | null;
};
...
export default function AdminLiveData() {
  ...
  async function load() {
    ...
    const [propertyResult, leadResult] = await Promise.all([
      ...
      supabaseBrowser.from("leads").select("id,name,phone,email,message,source,status,notes,created_at").order("created_at", { ascending: false }).limit(40),
    ]);
  ...
  const stats = useMemo(() => !isSupabaseConfigured ? {
    ...
    leads: leads.filter((lead) => lead.status !== "lost").length,
  ...
  async function saveEditing() {
    ...
    const payload = {
      title: editing.title, purpose: editing.purpose, segment: editing.segment || "residential", zone: editing.zone || "urban",
      publication_state: editing.publication_state || "published", status: editing.status, price: Number(editing.price || 0),
      bedrooms: Number(editing.bedrooms || 0), bathrooms: Number(editing.bathrooms || 0), parking_spaces: Number(editing.parking_spaces || 0),
      featured: Boolean(editing.featured), description: editing.description || null, address: editing.address || null, address_public: Boolean(editing.address_public),
  ...
  return (
    <>
      <div className="adminMetrics">
        ...
        <article><span>Contatos ativos</span><strong>{stats.leads}</strong><small>{isSupabaseConfigured ? "Pipeline comercial" : "Aguardando Supabase"}</small></article>
      </div>
      ...
      <div className="adminPanel" id="contatos"><div className="adminPanelHeader"><div><span className="eyebrow">LEADS</span><h2>Contatos recebidos</h2></div><span>{isSupabaseConfigured ? `${leads.length} recentes` : "Modo demonstração"}</span></div>
        {!isSupabaseConfigured ? <div className="emptyMini">Os contatos aparecerão aqui quando o Supabase estiver configurado.</div> : leads.length === 0 ? <div className="emptyMini">Nenhum contato recebido ainda.</div> : <div className="leadGrid">{leads.map((lead) => <article className="leadCard" key={lead.id}>
          <div className="leadHead"><strong>{lead.name || "Contato sem nome"}</strong><span>{new Date(lead.created_at).toLocaleDateString("pt-BR")}</span></div>
          <div className="leadPipeline"><select value={lead.status} onChange={(event) => void updateLead(lead.id, { status: event.target.value as LeadStatus })}>{Object.entries(leadLabels).map(([value,label]) => <option value={value} key={value}>{label}</option>)}</select></div>
          <p>{lead.message || "Sem mensagem."}</p>
          <div className="leadContacts">{lead.phone && <a href={`tel:${lead.phone}`}>{lead.phone}</a>}{lead.email && <a href={`mailto:${lead.email}`}>{lead.email}</a>}</div>
          <label className="leadNotes">Anotações<textarea rows={3} value={lead.notes || ""} onChange={(event) => setLeads((current) => current.map((item) => item.id === lead.id ? { ...item, notes: event.target.value } : item))} onBlur={(event) => void updateLead(lead.id, { notes: event.target.value })} placeholder="Observações do atendimento" /></label>
        </article>)}</div>}
      </div>
    </>
  );

...
const plans = [
  { code:"inicial", name:"Start", description:"Para começar com a estrutura essencial da imobiliária.", featured:false, monthly:39.90, annual:359.10, implementation:99, annualDiscount:25, limits:["Até 30 imóveis","1 usuário","2 acessos simultâneos","1 e-mail profissional","5 fotos por imóvel"], features:["Site exclusivo","Catálogo de imóveis","Leads via site e portais","Portais imobiliários","WhatsApp flutuante no site","Site otimizado para SEO","CRM com funil de vendas","Gestão de clientes","IA para descrições"] },
  { code:"profissional", name:"Pro", description:"Para quem precisa de gestão completa e mais capacidade.", featured:true, monthly:59.90, annual:539.10, implementation:99, annualDiscount:25, limits:["Até 400 imóveis","Até 3 usuários","6 acessos simultâneos","3 e-mails profissionais","50 fotos por imóvel"], features:["Site exclusivo","Catálogo de imóveis","Leads via site e portais","Portais imobiliários","Facebook Lead Ads","WhatsApp flutuante no site","Site otimizado para SEO","CRM com funil de vendas","CRM imobiliário completo","Gestão completa","Gestão de clientes","Gestão de vendas","Aplicativo do corretor","Controle de chaves","Controle de propostas","Agenda e visitas","Vistorias","IA para descrições"] },
  { code:"imobiliaria", name:"Business", description:"Para equipes com operação comercial, documentos e inteligência artificial.", featured:false, monthly:79.90, annual:719.10, implementation:100, annualDiscount:25, limits:["Até 1.000 imóveis","Até 5 usuários","10 acessos simultâneos","5 e-mails profissionais","50 fotos por imóvel"], features:["Site exclusivo","Catálogo de imóveis","Leads via site e portais","Portais imobiliários","Facebook Lead Ads","WhatsApp flutuante no site","Site otimizado para SEO","CRM com funil de vendas","CRM imobiliário completo","Gestão completa","Gestão de clientes","Gestão de vendas","Aplicativo do corretor","Controle de chaves","Controle de propostas","Agenda e visitas","Vistorias","IA para descrições","Reserva avançada de leads","Permissões por usuário","Relatórios estratégicos","Central de documentos","IA de oportunidades","Matching comprador × imóvel","Contato automático com consentimento"] },
  { code:"premium", name:"Prime", description:"Para operações maiores com domínio próprio, equipes e automações avançadas.", featured:false, monthly:110, annual:990, implementation:0, annualDiscount:25, limits:["Até 3.000 imóveis","Até 10 usuários","20 acessos simultâneos","10 e-mails profissionais","50 fotos por imóvel"], features:["Site exclusivo","Catálogo de imóveis","Leads via site e portais","Portais imobiliários","Facebook Lead Ads","WhatsApp flutuante no site","Site otimizado para SEO","CRM com funil de vendas","CRM imobiliário completo","Gestão completa","Gestão de clientes","Gestão de vendas","Aplicativo do corretor","Controle de chaves","Controle de propostas","Agenda e visitas","Vistorias","IA para descrições","Reserva avançada de leads","Permissões por usuário","Relatórios estratégicos","Central de documentos","IA de oportunidades","Matching comprador × imóvel","Contato automático com consentimento","Domínio próprio","Gestão multi-equipe","Maior capacidade de IA","Prioridade operacional","Recursos avançados de automação"] },
];
...
function money(value:number){return new Intl.NumberFormat("pt-BR",{minimumFractionDigits:2,maximumFractionDigits:2}).format(value);}
...
export default function PlatformLanding() {
  return (
    <main className="platformLanding">
      ...
      <section className="softSection" id="planos"><div className="container section"><div className="sectionHeading"><div><span className="eyebrow">PLANOS</span><h2>Escolha a estrutura certa para sua imobiliária.</h2><p className="sectionIntro">Veja os valores mensal e anual de cada plano e siga direto para a contratação.</p></div><div className="plansIntroActions"><a className="button secondary" href="#comparativo-planos">Comparar planos</a></div></div><div className="plansLegend"><span>✓ Incluído</span><span>– Não incluído</span><span>Limites podem ser ajustados no painel da plataforma</span></div><div className="plansGrid">{plans.map(plan=>{const free=plan.implementation<=0;return <article className={`planCard ${plan.featured?"featured":""}`} key={plan.name}>{plan.featured?<span className="planTag">MAIS EQUILIBRADO</span>:<span className="planTag">PLANO LENOY</span>}<h3>{plan.name}</h3><p className="standalonePlanSummary">{plan.description}</p><div className="standaloneCycleBox"><div><small>MENSAL</small><b>R$ {money(plan.monthly)}/mês</b>{free?<span>Implantação grátis</span>:<><span>Primeiro pagamento: somente implantação — R$ {money(plan.implementation)}</span><span>Você começa a pagar a mensalidade 30 dias após a contratação.</span></>}</div><a href={`/contratar/?plano=${encodeURIComponent(plan.code)}&ciclo=monthly`}>Contratar mensal</a></div><div className="standaloneCycleBox annual"><div><small>ANUAL · {plan.annualDiscount.toLocaleString("pt-BR")}% OFF</small><b>R$ {money(plan.annual)}/ano</b><span>Implantação grátis</span></div><a href={`/contratar/?plano=${encodeURIComponent(plan.code)}&ciclo=annual`}>Contratar anual</a></div><div className="planLimits">{plan.limits.map(item=><span key={item}>{item}</span>)}</div><div className="planFeatureList">{planBenefits.map(item=><span className={plan.features.includes(item)?"yes":"no"} key={item}>{item}</span>)}</div></article>})}</div>

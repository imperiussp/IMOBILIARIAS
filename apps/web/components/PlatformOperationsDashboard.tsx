...
type Metrics={agencies:number;pendingDomains:number;pendingInvites:number;overdueFollowups:number;unorganizedAssets:number;billingFailures:number;maintenanceFailures:number;lastMaintenance:string|null};
const empty:Metrics={agencies:0,pendingDomains:0,pendingInvites:0,overdueFollowups:0,unorganizedAssets:0,billingFailures:0,maintenanceFailures:0,lastMaintenance:null};
...
export default function PlatformOperationsDashboard(){
  ...
  useEffect(()=>{void(async()=>{ if(!supabaseBrowser){setLoading(false);return;} const now=new Date().toISOString(); const queries=await Promise.all([

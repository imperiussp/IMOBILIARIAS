...
export default function PlatformOperationsDashboard(){
  ...
  setMetrics({agencies:queries[0].count||0,pendingDomains:queries[1].count||0,pendingInvites:queries[2].count||0,overdueFollowups:queries[3].count||0,unorganizedAssets:unorganized,billingFailures:queries[5].count||0});setLoading(false);
  })();},[]);
  const attention=metrics.pendingDomains+metrics.overdueFollowups+metrics.unorganizedAssets+metrics.billingFailures;

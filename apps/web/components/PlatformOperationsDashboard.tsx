...
type Metrics={agencies:number;pendingDomains:number;pendingInvites:number;overdueFollowups:number;unorganizedAssets:number;billingFailures:number;maintenanceFailures:number;pendingProviderEvents:number;staleProviderEvents:number;lastMaintenance:string|null};
const empty:Metrics={agencies:0,pendingDomains:0,pendingInvites:0,overdueFollowups:0,unorganizedAssets:0,billingFailures:0,maintenanceFailures:0,pendingProviderEvents:0,staleProviderEvents:0,lastMaintenance:null};
...
export default function PlatformOperationsDashboard(){
  ...
  useEffect(()=>{void(async()=>{ if(!supabaseBrowser){setLoading(false);return;} const now=new Date().toISOString(); const staleCutoff=new Date(Date.now()-30*60*1000).toISOString(); const queries=await Promise.all([
    ...
    supabaseBrowser.from("outreach_provider_event_inbox").select("id",{count:"exact",head:true}).is("processed_at",null),
    supabaseBrowser.from("outreach_provider_event_inbox").select("id",{count:"exact",head:true}).is("processed_at",null).lt("received_at",staleCutoff),
  ]);
